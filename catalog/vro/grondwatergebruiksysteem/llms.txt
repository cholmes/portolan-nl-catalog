...
**Features:** 50,637  |  **CRS:** EPSG:4258  |  **License:** CC0 1.0 (public domain)
...
df = con.execute("""
    SELECT * FROM read_parquet('https://data.source.coop/cholmes/portolan-nl/vro/grondwatergebruiksysteem/grondwatergebruiksysteem.parquet') LIMIT 5
""").df()
```
...
gdf = gpd.read_parquet('https://data.source.coop/cholmes/portolan-nl/vro/grondwatergebruiksysteem/grondwatergebruiksysteem.parquet')   # CRS EPSG:4258
```
...
SELECT bro_id
FROM read_parquet('https://data.source.coop/cholmes/portolan-nl/vro/grondwatergebruiksysteem/grondwatergebruiksysteem.parquet')
WHERE ST_DWithin(
...
## Caveats
- Production/abstraction volumes are **not** in this collection — they are in the Groundwater Production Dossier (GPD), which has no own geometry and is published separately as a table.
- Detailed installations and facilities live in related tables in the source GeoPackage.

## Related collections
Groundwater production dossier (GPD, tabular), groundwater monitoring wells (GMW).
...
- **`styles/default.json`** — BRO Groundwater Use System (GUF / Grondwatergebruiksysteem) — Default (https://data.source.coop/cholmes/portolan-nl/vro/grondwatergebruiksysteem/styles/default.json)
- **`styles/by-delivery-context.json`** — BRO Groundwater Use System (GUF / Grondwatergebruiksysteem) — By delivery context (https://data.source.coop/cholmes/portolan-nl/vro/grondwatergebruiksysteem/styles/by-delivery-context.json)
