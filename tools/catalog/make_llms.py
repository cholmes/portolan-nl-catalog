#!/usr/bin/env python3
"""Generate comprehensive llms.txt for each VRO/BRO collection (and README via the same content).
...
"""
    with open(os.path.join(cdir, "llms.txt"), "w") as f:
        f.write(txt)
    print("  wrote", os.path.join("vro", path, "llms.txt"), f"({len(txt)} bytes)")
